...
export async function runCommand(text) {
  ...
  const prompt =
    ...
    `Rules: If he only wants advice, return actions as []. Resolve day words like "tomorrow" or "Thursday" using this list of upcoming dates: ${ctx.days.join(", ")}. ` +
  ...
  for (const a of actions) {
    try {
      if (a.type === "add_daily_task" && a.title) {
        await supabase.from("daily_tasks").insert({ title: a.title, day: today }); done++; didParts.push(`Added to today: "${a.title}".`);
      } else if (a.type === "add_weekly_task" && a.title) {
        await supabase.from("weekly_tasks").insert({ title: a.title }); done++; didParts.push(`Added weekly task: "${a.title}".`);
      } else if (a.type === "add_goal" && a.title) {
        const sc = a.scope === "week" ? "week" : "month";
        await supabase.from("goals").insert({ body: a.title, scope: sc }); done++; didParts.push(`Added ${sc} goal: "${a.title}".`);
      } else if (a.type === "add_event" && a.title && a.day) {
        const s = hhmmToMin(a.start) ?? 540; const e = hhmmToMin(a.end) ?? s + 60;
        await supabase.from("events").insert({ title: a.title, day: a.day, start_min: s, end_min: e }); done++;
        didParts.push(`Added event: ${a.title} on ${a.day} at ${a.start || "9:00"}.`);
      } else if (a.type === "add_recurring" && a.title) {
        const wd = wdIndex(a.weekday);
        if (wd >= 0) {
          await supabase.from("recurring_tasks").insert({ title: a.title, weekday: wd }); done++;
          didParts.push(`Repeating every ${WEEKDAYS[wd]}: "${a.title}".`);
        }
        ...
        if (hit) { await supabase.from("daily_tasks").update({ done: true }).eq("id", hit.id); done++; didParts.push(`Marked done: "${hit.title}".`); }
      } else if (a.type === "complete_weekly" && a.match) {
        const { data } = await supabase.from("weekly_tasks").select("id, title");
        const hit = (data || []).find((t) => t.title.toLowerCase().includes(String(a.match).toLowerCase()));
        if (hit) { await supabase.from("weekly_tasks").update({ done: true }).eq("id", hit.id); done++; didParts.push(`Marked done: "${hit.title}".`); }
      } else if (a.type === "pin_weekly" && a.match) {
        const { data } = await supabase.from("weekly_tasks").select("id, title");
        const hit = (data || []).find((t) => t.title.toLowerCase().includes(String(a.match).toLowerCase()));
        if (hit) { await supabase.from("weekly_tasks").update({ pinned: true }).eq("id", hit.id); done++; didParts.push(`Pinned to today: "${hit.title}".`); }
      }
    } catch (e) { /* skip a failed action, keep going */ }
  }

  // Build an honest reply that reflects what actually happened.
  if (actions.length > 0 && done === 0) {
    return { reply: "I understood that, but saving it failed. Your database may be missing a table. Try running the latest SQL update in Supabase.", done: 0 };
  }
  if (didParts.length) {
    return { reply: didParts.join(" "), done };
  }
