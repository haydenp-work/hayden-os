...
export async function runCommand(text) {
  ...
  for (const a of actions) {
    if (a.type === "add_daily_task" && a.title) {
      ...
      await run(supabase.from("meals").insert({ name: a.name, calories: Math.round(cals) || 0, protein: Math.round(prot) || 0 }).select().single(), `Logged: ${a.name} (${Math.round(prot) || 0}g protein).`, "meals");
    } else if (a.type === "add_protein" && a.grams) {
      const g = Math.round(Number(a.grams)) || 0;
      await run(supabase.from("meals").insert({ name: `${g}g protein`, calories: 0, protein: g }).select().single(), `Added ${g}g protein.`, "meals");
  ...
  if (errors.length && done === 0) {
    return { reply: `Save failed. Database said: ${errors[0]}`, done: 0, error: errors[0], created, completed, pinned, spendSet, proteinGoalSet };
  }
  if (didParts.length) {
    return { reply: didParts.join(" ") + (errors.length ? ` (one part failed: ${errors[0]})` : ""), done, created, completed, pinned, spendSet, proteinGoalSet };
  }
  return { reply: out.reply || "Okay.", done, created, completed, pinned, spendSet, proteinGoalSet };
}
