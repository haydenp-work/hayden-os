...
export async function runCommand(text) {
  ...
  // Safety net: if he clearly asked to add to today but the model produced no add, do it ourselves.
  ...
  if (wantsTodayAdd && !addedToday) {
    ...
    if (title) {
      const { error } = await supabase.from("daily_tasks").insert({ title, day: today });
      if (!error) { done++; didParts.push(`Added to today: "${title}".`); }
      else if (!errors.length) errors.push(error.message);
    }
  }
