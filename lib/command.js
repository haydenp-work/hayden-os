  ...
  for (const a of actions) {
    try {
      if (a.type === "add_daily_task" && a.title) {
        await supabase.from("daily_tasks").insert({ title: a.title, day: today }); done++;
      } else if (a.type === "add_weekly_task" && a.title) {
        await supabase.from("weekly_tasks").insert({ title: a.title }); done++;
      } else if (a.type === "add_goal" && a.title) {
        await supabase.from("goals").insert({ body: a.title, scope: a.scope === "week" ? "week" : "month" }); done++;
      } else if (a.type === "add_event" && a.title && a.day) {
        const s = hhmmToMin(a.start) ?? 540;
        const e = hhmmToMin(a.end) ?? s + 60;
        ...
        if (hit) { await supabase.from("daily_tasks").update({ done: true }).eq("id", hit.id); done++; }
      } else if (a.type === "complete_weekly" && a.match) {
        const { data } = await supabase.from("weekly_tasks").select("id, title");
        const hit = (data || []).find((t) => t.title.toLowerCase().includes(String(a.match).toLowerCase()));
        if (hit) { await supabase.from("weekly_tasks").update({ done: true }).eq("id", hit.id); done++; }
      } else if (a.type === "pin_weekly" && a.match) {
        const { data } = await supabase.from("weekly_tasks").select("id, title");
        const hit = (data || []).find((t) => t.title.toLowerCase().includes(String(a.match).toLowerCase()));
        if (hit) { await supabase.from("weekly_tasks").update({ pinned: true }).eq("id", hit.id); done++; }
      }
    } catch (e) { /* skip a failed action, keep going */ }
  }

  return { reply: out.reply || (done ? "Done." : "Okay."), done };
}
