...
export async function runCommand(text) {
  ...
  if (errors.length && done === 0) {
    return { reply: `Save failed. Database said: ${errors[0]}`, done: 0, error: errors[0], created, completed, pinned };
  }
  if (didParts.length) {
    return { reply: didParts.join(" ") + (errors.length ? ` (one part failed: ${errors[0]})` : ""), done, created, completed, pinned };
  }
  return { reply: out.reply || "Okay.", done, created, completed, pinned };
}
