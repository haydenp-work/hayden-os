...
// Send an image plus a prompt to Claude (vision) and return the text answer.
export async function askClaudeImage(prompt, imageBase64, mediaType, { maxTokens = 600 } = {}) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: {
      "content-type": "application/json",
